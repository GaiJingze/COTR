    ...
    def conclude_tasks(self, tasks, return_idx=False, force=False,
        ...
        if corrs.shape[0] > 0:
            ...
            if img_a_shape is not None and img_b_shape is not None and not force:
                ...
                border_mask = (np.prod(corrs > np.array([0, 0, 0, 0]), axis=1) * border_mask).astype(np.bool_)
        ...
        if queries_a is None:
            ...
            if force:
                for i, loc_from in enumerate(queries_a):
                    pos = loc_from[::-1]
                    pos = np.array([np.clip(pos[0], 0, corr_a.shape[0] - 1), np.clip(pos[1], 0, corr_a.shape[1] - 1)], dtype=np.int_)
